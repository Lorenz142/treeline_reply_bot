...
def get_my_user_id():
    ...
    print(f"Failed to get user ID: {resp.status_code} {resp.text}")
    return None


def like_tweet(tweet_id):
    """Like a tweet to establish engagement."""
    ...
    if resp.status_code in (200, 201):
        ...
        print(f"Failed to like tweet: {resp.status_code} {resp.text}")
        return False


def post_reply(tweet_id, reply_text, author_username=None):
    """Post a reply to a tweet on X using OAuth 2.0. Likes the tweet first to establish engagement."""

    # Like the tweet first to satisfy X's engagement requirement
    like_tweet(tweet_id)
    time.sleep(1)
