...
def get_list_tweets():
    ...
    try:
        response = x_client.get_list_tweets(
            id=LIST_ID,
            max_results=20,
...
def post_reply(tweet_id, reply_text, author_username=None):
    ...
    try:
        ...
        print(f"Reply posted to tweet {tweet_id}: {response.data}")
        return True
    except tweepy.errors.Forbidden as e:
        print(f"Forbidden posting reply: {e}")
        # Try with @mention prepended
        if author_username and f"@{author_username}" not in reply_text:
            try:
                print(f"Retrying with @{author_username} mention...")
                response = x_client.create_tweet(
                    text=f"@{author_username} {reply_text}",
                    in_reply_to_tweet_id=tweet_id,
                )
                print(f"Reply posted with mention to tweet {tweet_id}: {response.data}")
                return True
...
def test_post():
    ...
    try:
        response = x_client.create_tweet(text="test — ignore")
        tweet_id = response.data["id"]
        print(f"Test tweet posted: {tweet_id}")
        # Delete it immediately
        x_client.delete_tweet(tweet_id)
...
def test_reply():
    ...
    try:
        # Post a tweet
        response = x_client.create_tweet(text="test thread — ignore")
        tweet_id = response.data["id"]
        print(f"Test tweet posted: {tweet_id}")

        # Try to reply to it
        reply_response = x_client.create_tweet(
