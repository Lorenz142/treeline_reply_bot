...
# ── Tweepy client for POSTING (OAuth 2.0 user token) ──
write_client = tweepy.Client(oauth2_access_token)


def refresh_oauth2_token():
    """Refresh the OAuth 2.0 access token using the refresh token."""
    global oauth2_access_token, oauth2_refresh_token, write_client
...
def test_post():
    ...
    try:
        response = write_client.create_tweet(text="test — ignore")
        tweet_id = response.data["id"]
        print(f"Test tweet posted: {tweet_id}")
        # Delete it immediately
        write_client.delete_tweet(tweet_id)
...
def test_reply():
    ...
    try:
        # Post a tweet
        response = write_client.create_tweet(text="test thread — ignore")
        tweet_id = response.data["id"]
        print(f"Test tweet posted: {tweet_id}")

        # Try to reply to it
        reply_response = write_client.create_tweet(
        ...
        # Clean up
        write_client.delete_tweet(reply_id)
        write_client.delete_tweet(tweet_id)
